<?php

return [
    "regards" => "au plaisir,",
    "befbtn" => "Vous recevez ce courriel, car nous avons reçu une demande de changement de mot de passe pour votre compte.",
    "aftbtn" => "Ce lien de réinitialisation de mot de passe expirera dans 60 minutes.\n\n",
    "befsal" => "Si vous n'avez pas envoyé la demande de changement de mot de passe, vous pouvez ignorer ce courriel.",
    "btn" => "Réinitialiser le mot de passe",
    "trouble" => "Si vous avez des problèmes en avec le bouton \"Réinitialiser le mot de passe\", copier et coller ce lien dans votre navigateur=>",
    "subject" => "Confirmation de votre commande",

    "bonjour" => "Bonjour",
    "merci" => "Merci pour votre commande ! Vous trouverez ci-dessous votre facture détaillée.",
    "interac1" => "Vous pouvez l'acquitter par virement Interac à l'adresse courriel",
    "interac2" => "La réponse à la question de sécurité doit être ",
    "glaciere" => "Merci de laisser une glacière de taille appropriée visible à l'extérieur pour votre commande.",
    "appt" => "Appt",
    "commande" => "Commande",
    "livraison" => "Livraison",
    "cueillette" => "Cueillette",
    "allergenes" => "Allergènes",
    "sous_total" => "Sous-total",
    "semaine" => "Passez une excellente semaine et merci d'acheter local !",
    "chef" => "Chef exécutif",
    "en_ligne" => "Payée en ligne",
    "separement" => "Payée séparément"
];
